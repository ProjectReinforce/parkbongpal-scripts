...
public class ReinforceDetailUI : MonoBehaviour
{
    ...
    void Refresh()
    {
        ...
        texts[(int)StatType.atk-1].text = $": {data.atk}\n({data.defaultStat[(int)StatType.atk]} <color=red>+{data.PromoteStat[(int)StatType.atk]}</color> <color=green>+{data.AdditionalStat[(int)StatType.atk]}</color> <color=blue>+{data.NormalStat[(int)StatType.atk]}</color> <color=cyan>+{data.SoulStat[(int)StatType.atk]}</color> <color=yellow>+{data.RefineStat[(int)StatType.atk]}</color>)";
